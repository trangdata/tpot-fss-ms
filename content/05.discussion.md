...
For example, for high-dimensional data, dimension reduction algorithms or feature selection algorithms are preferred to add into pipeline via Template for reducing computation time and potentially detecting important features.
For dataset mixed with categorical features, preprocessing operators for encoding those features, like one-hot encoder, should be specified in pipeline structure for improving pipelines' performance. In this study, the Dataset Selector was specified as the first step of pipeline via Template, which enables the comparison between the two TPOT implementations, with and without DS.
